...
void QCPDataSelection::simplify() {
    ...
    int i = 1;
    while (i < mDataRanges.size()) {
        if (mDataRanges.at(i - 1).end() >= mDataRanges.at(
                i).begin()) // range i overlaps/joins with i-1, so expand range i-1 appropriately and remove range i from list
        {
...
QCPDataSelection QCPDataSelection::intersection(const QCPDataRange &other) const {
    QCPDataSelection result;
            foreach (QCPDataRange dataRange, mDataRanges)result.addDataRange(dataRange.intersection(other), false);
    result.simplify();
    return result;
}
